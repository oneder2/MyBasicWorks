...
def check_keydown_events(event, ai_settings, screen, stats, sb, 
                         ship, aliens, ship_bullets, alien_bullets):
    """相应按键"""
    if event.key == pygame.K_RIGHT:
        ...
        # 开始游戏快捷键
        start_game(ai_settings, screen, stats, sb, ship, aliens,ship_bullets, alien_bullets)


def check_keyup_events(event, ai_settings, screen, ship, ship_bullets, alien_bullets):
    """相应松开"""
    if event.key == pygame.K_RIGHT:
        # 停止向右移动飞船
...
def check_events(ai_settings, screen, stats, sb, play_button, 
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keyup_events(event, ai_settings, screen, ship, ship_bullets, alien_bullets)
        elif event.type == pygame.MOUSEBUTTONDOWN:
            mouse_x, mouse_y = pygame.mouse.get_pos()
            check_play_button(ai_settings, screen, stats, sb, play_button, 
                              ship, aliens, ship_bullets, alien_bullets, mouse_x, mouse_y)
...
def start_game(ai_settings, screen, stats, sb, ship, aliens, ship_bullets, alien_bullets):
    """开始游戏"""
    # 隐藏光标
...
def update_screen(ai_settings, screen, stats, sb, 
                  ship, aliens, ship_bullets, alien_bullets, play_button):
    """更新屏幕上的图像，并切换到新屏幕"""
    # 每次循环时都重绘屏幕
...
def fire_bullet_ship(ai_settings, screen, ship, aliens, ship_bullets, alien_bullets):
    """如果还没有达到限制，发射一颗子弹"""
    # 创建一颗子弹，并将其添加到分组Group中
    if len(ship_bullets) < ai_settings.bullet_allowed and ship.fire == True:
        new_bullet = Bullet(ai_settings, screen, ship, aliens)
        ship_bullets.add(new_bullet)


def fire_bullet_alien(ai_settings, screen, aliens, ship_bullets, alien_bullets):
    """外星人循环发射子弹"""
    # 创建一颗子弹，并将其添加到分组Group中
    new_bullet = alien_bullets(ai_settings, screen, aliens)
...
def check_bullet_ship_collisions(ai_settings, screen, stats, sb, 
    ...
    collisions = pygame.sprite.groupcollide(alien_bullets, aliens, True, True)
...
def update_bullets(ai_settings, screen, stats, sb, 
    ...
    """更新子弹位置，并删除已经消失的子弹"""
    #外星人发射子弹
    # fire_bullet_alien(ai_settings, screen, aliens, alien_bullets)

    # 更新子弹位置
    for bullet in ship_bullets:
        bullet.update_ship()
    # alien_bullets.update_aliens()
